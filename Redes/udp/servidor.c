...

{
	...
	server.sin_port = htons(atoi(argv[1])); //Guarda el numero de puerto de recepcion
	//atoi convierte un char a entero
	//htons convierte el valor argv que es recibido por parametro en un num de puerto
	
	server.sin_addr.s_addr = INADDR_ANY; //Guarda la direccion por la que se va a recibir
	...
	while(true){ //Esta siempre recibiendo conexiones
      ...
      exit(-1);
		...
		if(strcmp(buf, "fecha") == 0){
			strcpy(buf, getFecha());
			sendto(fd1, buf, 12, 0, (struct sockaddr*)&client, size);
		}else if(strcmp(buf, "hora") == 0){
			strcpy(buf, getHora());
			sendto(fd1, buf, 10, 0, (struct sockaddr*)&client, size);
		}else{
			sendto(fd1, "ERROR: - Comando incorrecto\r\n", 29, 0, (struct sockaddr*)&client, size); 
		}
	}
