...
// Pre: h no esta lleno.
	...
	int swap;
	h->tope++;
	while((pos > 0) && (h->arr[pos] < h->arr[(pos-1)/2])){ //Mientras el elemento sea mas chico que su padre y la posicion sea mmayor que cero
		swap = h->arr[pos]; //Guardo el item 
		h->arr[pos] = h->arr[(pos-1)/2]; //Guardo al padre en la posicion del item
		h->arr[(pos-1)/2] = swap; //Guardo al item en la posicion del padre
		pos = (pos-1)/2;
	}
	return h;
}
...
// Pre: h tiene elementos.
	...
	int pos = 0, swap, pos_swap;
	bool intercambiar = true;
	while((intercambiar == true) && (2*pos+1 < h->tope)){ //Mientras tenga al menos un hijo
		//Busco al hijo más chicx
		if(2*pos+2 < tope){ //Si tiene dos hijos
			if(h->arr[2*pos+1] < h->arr[2*pos+1]){ //Si el hijo de la izquierda es menor
				pos_swap = 2*pos+1;
			}else{
				pos_swap = 2*pos+2;
			}
		...
		//Si soy mayor que mi hijo mas chico, me intercambio
		if(h->arr[pos] > h->arr[pos_swap]){
			swap = h->arr[pos];
			h->arr[pos] = h->arr[pos_swa];
			h->arr[pos_swap] = swap;
			pos = pos_swap;
		}else
			intercambiar = false; //Soy el mas chico de mis hijos
	}
	return h;
}
